...
const buildWAMsg = (d, r, rate, s) => [
  `🌐 *PRESUPUESTO DE IMPORTACIÓN*`, `*FVR Logística Internacional*`, ``,
  `👤 *Cliente:* ${d.nombre}`,
  `📱 *WhatsApp:* ${d.whatsapp}`,
  `📧 *Email:* ${d.email || "—"}`,
  `📦 *Producto:* ${d.producto}`,
  `🔢 *HS Code:* ${d.hsCode || "No indicado"}`,
  d.aiSuggestion ? `🤖 *Análisis IA:* ${d.aiSuggestion}` : "",
  ``,
  `🚀 *Tipo:* ${d.tipo === "avion" ? `✈️ Avión — ${d.subTipo === "personal" ? "Envío Personal (Franquicia)" : "Envío Comercial"}` : "🚢 Barco"}`,
  `💵 *FOB / Valor productos:* ${USD(r.fob)}`,
  r.isAir
    ? `⚖️ Peso real: ${r.peso} kg | Volumétrico: ${fmt(r.pVol)} kg | Facturable: *${fmt(r.pFact)} kg*`
    : `📐 Volumen: ${fmt(r.m3, 3)} m³ | Facturable: *${fmt(r.m3Fact, 3)} m³*`,
  ``,
  `📊 *COSTOS DETALLADOS*`,
  ...
  r.isPersonal
    ...
    : `• Derecho de importación (${r.effectiveDutyPct}%${d.aiDutyRate !== null ? " — vía IA" : ""}): ${USD(r.duty)}`,
  r.isPersonal ? `• Tasa estadística: No aplica` : `• Tasa estadística (${s.stat}%): ${USD(r.stat)}`,
  ...
  `• Envío nacional: ${USD(r.domestic)}`,
  `• Honorarios de gestión (${s.feeType === "fixed" ? "fijo" : `${s.feePct}% s/${s.feeBase === "fob" ? "FOB" : "costos"}`}): ${USD(r.fees)}`,
  ``,
  `═══════════════════════`,
  `📋 Total envío (sin producto): *${USD(r.totalLog)}*`,
  `💰 *TOTAL GENERAL: ${USD(r.totalGen)}*`,
  rate ? `💱 *En pesos: ARS ${fmt(r.totalGen * rate, 0)}* (dólar $${fmt(rate)})` : "",
  d.files?.length ? `📎 Archivos: ${d.files.join(", ")}` : "",
  ``,
  `_FVR Logística Internacional_`,
  `_Francisco Vega · +54 9 3885 223299_`,
...
const generatePDFHTML = (d, r, dolar, s) => {
  const tipo = d.tipo === "avion"
    ? `✈️ Avión — ${d.subTipo === "personal" ? "Envío Personal (Franquicia)" : "Envío Comercial"}`
    : "🚢 Barco";
...
<div class="sec">
  ...
  <div class="row hi"><span>Derecho de importación (50% sobre excedente)</span><span>${USD(r.duty)}</span></div>
  ...
  <div style={{ marginTop:16 }}>
    ...
    {value === "personal" && (
      <div style={{ marginTop:10, background:"#fffbeb", border:"1px solid #fde68a", borderRadius:12, padding:12, fontSize:12, color:"#92400e" }}>
        <strong>🏷️ Franquicia personal:</strong> Sin derechos hasta USD 400. Si supera, se aplica <strong>50% sobre el excedente</strong>. Tasa estadística e IVA <strong>no aplican</strong>.
      </div>
    )}
  </div>
);
...
1. El código HS (arancelario) más probable para importación a Argentina
2. El porcentaje de derecho de importación aplicable (según nomenclatura arancelaria argentina SIM/VUCE)
3. Una descripción breve del producto
    ...
    <div style={{ minHeight:"100vh", background:"#f0f4f8" }}>
      ...
      <main style={{ maxWidth:640, margin:"0 auto", padding:"24px 16px" }}>
        ...
        <Card icon="📦" title="Datos del producto">
          ...
          <div style={rowS}>
            <Field label="HS Code / Código arancelario" hint="Se completa automáticamente con la IA">
    ...
    <div style={{ minHeight:"100vh", background:"#f0f4f8" }}>
      ...
      <main style={{ maxWidth:640, margin:"0 auto", padding:"24px 16px" }}>
          ...
          <div style={{ background:"#fffbeb", border:"1px solid #fbbf24", borderRadius:14, padding:"12px 16px", marginBottom:16, display:"flex", gap:10 }}>
            ...
            <div>
              ...
              <p style={{ fontSize:12, color:"#b45309" }}>
                {r.fob <= 400 ? "FOB ≤ USD 400 — sin derechos de importación." : `Excedente de USD 400: ${USD(r.fob - 400)} → Derecho (50%): ${USD(r.duty)}`}
                {" "}Tasa estadística, IVA y demás impuestos no aplican.
              </p>
            </div>
          </div>
        )}
        ...
        <Card icon="🏛️" title="Tributos aduaneros">
          {r.isPersonal ? (<>
            <Row label="Derecho de importación (franquicia personal)" usd={r.duty} dolar={dolar}
              note={r.fob <= 400 ? "FOB ≤ USD 400 — sin derecho" : `50% sobre USD ${fmt(r.fob - 400)} excedente`} />
            <Row label="Tasa estadística" na />
        ...
        <Card icon="🚚" title="Servicios logísticos">
          ...
          <Row label={`Honorarios de gestión (${s.feeType === "fixed" ? "fijo" : `${s.feePct}% s/${s.feeBase === "fob" ? "FOB" : "costos"}`})`} usd={r.fees} dolar={dolar} hi />
        </Card>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:24 }}>
        ...
        {tab === "settings" && (
          <div>
            ...
            <Card icon="🚚" title="Servicios logísticos">
              <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
                <SF label="Pick up (USD)" k="pickup"/><SF label="Handling (USD)" k="handling"/><SF label="Envío nacional (USD)" k="domestic"/>
              </div>
